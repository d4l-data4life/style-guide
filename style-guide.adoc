...
((&#10006;)) Remove the users from the group. Afterwards, they cannot access the resource anymore.

((&#10004;)) Remove the users from the group. Afterwards, the users cannot access the resource anymore.
...
It's OK to use "please" in situations where users are asked to do something inconvenient.
...
((&#10004;)) 9% of our test users always scanned any new page they came across. Only 16% read word by word.
...
- Ensure that the purpose of the list is clear in the heading and in the introduction.
...
TIP: Lean more" is the new "Click here". Be more descriptive in the link text, unless you have no space. Just using "Learn more" as link text has usability and accessibility issues. 
...
|button 
|choose
|if choose sounds awkward: click or tap, without _on_. But click or tap limits your description to a subset of devices. Don't use _select_ for buttons. You "select" one out of several options, or from several files. 
...
|date input field
|enter
|it's a field not a box, in the future it might be a date picker. Use enter or just use, avoid type.
...
|icon   
|choose
|an icon ist like a button, if choose sounds awkward: click or tap , without "on"
...
|menu
|
|menus contain _commands_ that users _choose_ in the menu. Don't refer to a command as a menu item, a choice, an entry, or an option. Submenu one word.
...
|some call it switch, users know hot it works, so you don't have to use verbs like "toggle" or "slide".

|===

IMPORTANT: For UI elements without interaction, use _is displayed_. It's OK to use the passive voice here because the actor is obvious. Avoid the following words: displays, appear, opens, or shows.

NOTE: For end user documentation, refer to unavailable commands and options on the UI as "unavailable", not as "grayed out", "dimmed", "disabled", or "inactive". Google calls these commands "disabled". Apple accepts disabled for developer content. Avoid "disabled" in content for end users.
